...
<br>
	If you find yourself using it a lot you might want to <a href="https://www.paypal.com/cgi-bin/webscr?cmd=_xclick&business=paypal@maiq.info&currency_code=EUR&amount=&return=&item_name=MQ Relinks plugin" target="_new"><img src="../wp-content/plugins/mq-relinks/donate.gif"></a>
<br>
...
function mq_ReLinks_replaceit_post($matches) {
	...
	if ($mq_relinks_options_value['blank_in_post'] =='yes'){ if (($local === false) && ($href{6}!="#")) $blank = 'target="_blank"';}
...
function mq_ReLinks_replaceit_comment($matches) {
	...
	if ($href) {
	$local = strpos($href, $burl);
	if ($mq_relinks_options_value['blank_in_comment'] =='yes'){if (($local === false) && ($href{6}!="#")) $blank = 'target="_blank"';} 
...
function mq_auth_ReLinks($link) {
	...
	if ($mq_relinks_options_value['blank_in_author_comment_link'] =='yes'){if (($local === false) && ($href{6}!="#")) $blank = 'target="_blank"';}
